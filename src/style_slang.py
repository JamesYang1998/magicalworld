...
def get_random_slang(category: str = None, probability: float = 0.15) -> str:
    ...
    for cat in professional_categories:
        if cat in CRYPTO_SLANG:
            all_slang.extend(CRYPTO_SLANG[cat])
...
def enhance_response(response: str, tweet_text: str) -> str:
    ...
    # Define word lists for pattern matching
    morning_words = ['gm', 'morning', 'hello', 'hi', '早上好']
    market_words = ['market', 'price', 'analysis', 'trend', 'bull', 'bear']

    # Add natural greeting for morning tweets
    if (available_chars > 5 and
            any(word in tweet_lower for word in morning_words)):
    ...
    if (available_chars > 2 and
            any(word in tweet_lower for word in market_words)):
    ...
    response = response.replace('!', '.')  # Replace exclamation marks
    response = ' '.join(
        word for word in response.split()
        if not word.startswith('#')  # Remove hashtags
    )

    # Remove emojis (covering the full Unicode range)
    response = ''.join(
        c for c in response
        if not (
            '\U0001F300' <= c <= '\U0001F9FF' or  # Misc Symbols and Pictographs
            '\U0001F600' <= c <= '\U0001F64F' or  # Emoticons
            '\U0001F680' <= c <= '\U0001F6FF' or  # Transport and Map Symbols
            '\U0001F900' <= c <= '\U0001F9FF' or  # Supplemental Symbols
    ...
    if random.random() < 0.15 and len(response) < 240:
        questions = [
