"""Tests for style matching and language detection functionality."""
...
def test_english_tweet_response():
    ...
    mock_response.choices = [
        Mock(message=Mock(content="Market looks strong. What's your take on it?"))
    ]
...
def test_mixed_language_response():
    ...
    mock_response = Mock()
    mock_response.choices = [
        Mock(message=Mock(content="是的，DeFi很有发展。你怎么看？"))
    ]
    ...
    with patch('src.llm.OPENAI_API_KEY', 'test_key'), \
        ...
        call_args = mock_client.chat.completions.create.call_args[1]
        assert "Reply in both Chinese and English" in \
...
def test_conversational_elements():
    ...
    mock_response.choices = [
        Mock(message=Mock(
            content="Good move. How long have you been investing in crypto?"
        ))
    ]
    ...
    with patch('src.llm.OPENAI_API_KEY', 'test_key'), \
        ...
        assert not any(char in response for char in ['!', '#']), \
            "Response should not include ! or #"
        assert not any(c for c in response if '\U0001F300' <= c <= '\U0001F9FF'), \
            "Response should not include emojis"
...
def test_response_length():
    ...
    mock_response = Mock()
    mock_response.choices = [
        Mock(message=Mock(content="A" * 300))  # Deliberately too long
    ]
    ...
    with patch('src.llm.OPENAI_API_KEY', 'test_key'), \
        ...
        mock_client.chat.completions.create.return_value = mock_response
        response = generate_response(tweet)
        assert len(response) <= 280, "Response should not exceed Twitter limit"
...
def test_market_context_influence():
    ...
    mock_response.choices = [
        Mock(message=Mock(
            content="Bitcoin moving past $40k with increasing alt season momentum"
        ))
    ]
    ...
    with patch('src.llm.OPENAI_API_KEY', 'test_key'), \
            patch('src.llm.client') as mock_client:
        mock_client.chat.completions.create.return_value = mock_response
        context = "Bitcoin at $40k, alt season hype"
        response = generate_response(tweet, market_context=context)
        assert any(term in response.lower() for term in ['40k', 'alt season']), \
            "Response should reflect market context"
